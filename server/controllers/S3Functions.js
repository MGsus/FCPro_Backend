...
var config = {
  ...
  credentials: new AWS.Credentials(
    IBMCloudEnv.getString("access_key_id"),
    IBMCloudEnv.getString("secret_access_key"),
    (sessionToken = null)
  ),
  signatureVersion: "v4",
};
...
module.exports = {
  ...
  getURL: function (bucket, key) {
    ...
    return s3
      .getSignedUrl("getObject", { Bucket: bucket, Key: key })
      .promise()
      .then(function (url) {
  ...
  copyItem: function (bucketDest, bucketOrigin, itemName) {
    ...
    parametros = {
      Bucket: bucketDest,
      CopySource: bucketOrigin + "/" + itemName,
      Key: itemName,
    };
    console.log(parametros);
    return s3
      .copyObject(parametros)
      .promise()
      .then(() => {
